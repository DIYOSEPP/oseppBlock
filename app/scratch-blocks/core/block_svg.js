...
Blockly.BlockSvg.prototype.handleDragFree_ = function (oldXY, newXY, e) {
    ...
    if (closestConnection && closestConnection.type == Blockly.PREVIOUS_STATEMENT) {
        if (localConnection && !localConnection.sourceBlock_.previousConnection) {
            if (closestConnection.targetConnection) {
                updatePreviews = false;
            }
        }
    }
    if (localConnection && localConnection.type == Blockly.PREVIOUS_STATEMENT) {
        if (closestConnection && !!closestConnection.sourceBlock_.previousConnection) {
            if (localConnection.targetConnection) {
                updatePreviews = false;
            }
        }
    }
...
Blockly.BlockSvg.prototype.removeInput = function (name, opt_quiet) {
    ...
    var node = this.inputShapes_[name];
    if (node && node.parentNode) {
        node.parentNode.removeChild(this.inputShapes_[name]);
        delete this.inputShapes_[name];
    }
    if (this.rendered) {
        this.render();
