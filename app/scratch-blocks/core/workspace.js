...
Blockly.Workspace.prototype.undo = function(redo) {
  ...
  try {
    ...
    //When redoing all blocks is disrupted, the temporary solution is to re-render them
    function re_render(block) {
        block.render();
        for (var c = 0, cb; cb = block.childBlocks_[c]; c++)re_render(cb);
    }
    var rended = [];
    for (var i = 0, event; event = events[i]; i++) {
        ...
        if (block) {
            if (rended.indexOf(block) >= 0) continue;
            re_render(block);
            rended.push(block);
        }
    }
